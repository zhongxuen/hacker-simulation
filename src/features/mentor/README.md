...
- **`server.ts` (server-only)** — for the routes and tests only.
  ...
  - `buildHintPrompt`, `buildExplainPrompt`, `buildReviewPrompt` — pure prompt building. Versioned prompt files are in `prompts/` (`hint.v2`, `explain.v1`, `review.v1`, and `noor.v1`, the persona, voice, safety and data rules they share). Prompts are versioned files, never edited in place: `hint.v1` is superseded by `hint.v2` (which puts the mission's difficulty in front of the model) and is kept unchanged for reference.
