...
- `components/` — `Terminal` (header with the SIMULATED badge, Help menu, Copy transcript, Reset machine; the output log; chips; the prompt), `OutputBlock` (with `PromptLabel` in the learner's prompt style), `PromptInput` (a real input over a mirror that draws the cursor, via `CursorMark`, and ghost text), `TerminalTour`, `ShortcutsDialog`, `CommandCheatSheet`, `TerminalPreview` (a few lines in any theme, prompt and cursor style, drawn with the same pieces) and `TerminalLookSettings` (the `/settings` card for the terminal's look). Components import nothing from `@/sim` at runtime (a test checks).
